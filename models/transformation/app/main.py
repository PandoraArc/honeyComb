...
async def predict_smiles(file: UploadFile = File(...)):
    ...
    try:
        ...
        smiles_prediction_mod = smiles_prediction.replace("=","eq").replace("(","l").replace(")","r")
        ...
        tags = {
            ...
            "smile_string": smiles_prediction_mod,
        }
        
        file_ext = os.path.splitext(file.filename)[1]
        filename = f"transformation/{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}{file_ext}"
