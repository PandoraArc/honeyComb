...
def main(smile_path, base_image_path, save_folder, epochs, n_heads, n_rep, tokenizer_path=None,):
    
    print("smile_path", smile_path)
    print("base_image_path", base_image_path)
    print("save_folder", save_folder)
    print("epochs:",epochs)
    print("number of n_heads",n_heads)
    print("number of replication:",n_rep)
...
#create TFracored from images
    ...
    # Set Training Epochs
    EPOCHS = epochs
    REPLICA_BATCH_SIZE = n_rep #! default 64
    ...
    def get_dataset(batch_size=BATCH_SIZE, buffered_size=BUFFER_SIZE, path=""):
        ...
        print(len(filenames))
        fx = open(f"{save_folder}/filenames", "w")
    ...
    # Path to checkpoints
    checkpoint_path = f"{save_folder}/checkpoints_SMILES_GPU/"
    ...
    val_loss_plot = []
    val_acc = []
    f = open(f"{save_folder}/Training_SMILES_GPU.txt", "w")
    sys.stdout = f
    ...
    plt.savefig(f"{save_folder}/Lossplot_SMILES_GPU.jpg")
    plt.close()
    ...
    plt.gcf().set_size_inches(20, 20)
    plt.savefig(f"{save_folder}/accuracyplot_SMILES_GPU.jpg")
    plt.close()

    f.close()
    
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Training transformation model")
    ...
    main(smile_path, base_image_path, save_folder, epochs, n_heads, n_rep, tokenizer_path)
    
    duration = datetime.now() - start_time

    #record paramaters
    output_file = f"{save_folder}/paramaters.txt"

    with open(output_file, "w") as f:
        f.write(f"smile_path: {smile_path}\n")
        f.write(f"base_image_path: {base_image_path}\n")
        f.write(f"save_folder: {save_folder}\n")
        f.write(f"epochs: {epochs}\n")
        f.write(f"number of n_heads: {n_heads}\n")
        f.write(f"number of replication: {n_rep}\n")
